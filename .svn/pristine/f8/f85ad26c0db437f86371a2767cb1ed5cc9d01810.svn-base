...
io.sockets.on('connection', function (socket) {
    ...
    socket.on('getComments', function (data) {
    	data.article_id = tools.sanitize(data.article_id);
    	
    	if(config.general.debug) {
    		console.log('Getting list for Comments for the Article - ' + data.article_id);
    	}
    	
    	// get article comment list
    	getComments(socket,data.article_id);
    });
...
function getComments(socket, article_id) {
	var connection = mysql.createConnection(db_access);
	connection.connect();
	
	var q = "SELECT * FROM " + tbl_comments + " WHERE (`article_id` = '" + [article_id] + "')";
	
	connection.query(q, function (qe, qr){
		if(qe && config.general.debug) { console.log(qe); }
		
		for(c_data in qr) {
			qr[c_data].time = qr[c_data].created_at;
			qr[c_data].nick = qr[c_data].sender;
			qr[c_data].comment = qr[c_data].body;
			
			io.sockets.emit("add_comment", {
				comment_data: qr[c_data]
			});
		}
